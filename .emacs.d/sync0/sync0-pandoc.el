...
;;   (let* ((defaults-file (expand-file-name (concat "defaults_" type ".yaml") sync0-pandoc-dir-cache))
;;          ;; Define a metadata file name with a 'metadata_' prefix
;;          (metadata-file (expand-file-name (concat "metadata_" type ".yaml") sync0-pandoc-dir-cache))
    ...
    (with-temp-buffer
      ...
      (if (re-search-forward "^export_template: \\([[:graph:]]+\\)$" nil t)
		 ...
		 (lang (sync0-pandoc-get-yaml-property "lang" "en-US"))
                 (yaml-cache-dir (expand-file-name sync0-pandoc-dir-cache))
...
(setq sync0-pandoc-org-to-docx-command
      (concat
       "pandoc"
       " --from=org --to=docx"
       " --standalone"
       ...
       " --bibliography=/home/sync0/Gdrive/bibliographies/master.bib"
       " --reference-doc=/home/sync0/Gdrive/typography/pandoc/conference_template.docx"
       " --csl=/home/sync0/Gdrive/typography/csl/chicago-author-date-fr-sync0.csl" 
       " --metadata=reference-section-title:References"
       ;; These two filters have been tested and dont worky yet
       ;; " --lua-filter=remove-org-tags.lua"
       ;; " --lua-filter=remove-org-ignore-headings.lua"
       " --lua-filter=delink.lua"
       " --citeproc"
       ))

(defun sync0-pandoc-export-org-to-docx ()
  "Convert an Org file to DOCX using Pandoc.
   Handles both structured (with YAML export_template) and unstructured exports."
  (interactive)
  (let* ((current-file (buffer-file-name))
         (previous-file sync0-pandoc-export-previous-file)
         (file (if (string= current-file previous-file)
                   sync0-pandoc-export-previous-file
                 (if (and current-file (string-match "\\.org\\'" current-file))
                     (if (y-or-n-p (format "Run Pandoc on the current buffer file: %s? " current-file))
                         current-file
                       (read-file-name "Choose an Org file: " nil nil t))
                   (read-file-name "Choose an Org file: " nil nil t))))
         (command (format "%s %s -o %s.docx"
                          sync0-pandoc-org-to-docx-command file (file-name-sans-extension file))))
          (shell-command command)
          (setq sync0-pandoc-export-previous-file file)
          (message "Pandoc DOCX command executed on %s" file)))
