...
;;;     FAX             = "+1 801 581 4148",
;;;     checksum        = "31062 4007 15559 134865",
...
(defconst bibtools-version "1.91 [24-Nov-2018]"
...
;;;
;;; 1.91 [24-Nov-2018]
;;;	Add new substitutions in bibtex-title-abbrev from
;;;     biblabel-0.09  [21-Sep-2015].
;;;
;;; 1.90 [25-May-2014]
...
(defvar bibtex-title-word-ignore-list
  '(
    ...
    ;; More words added [21-Sep-2015]
    "bioname"
    "booktitle"
    "ueber"				;used in early 20th Century German journals
)
  ...
  (let ((abbrev nil) (begin) (s) (word) (wordlist))
	...
	(progn
	  ...
	  (if (re-search-forward "\",[ \t]*$" nil t)
		...
		;; Add 11 substitutions from biblabel-0.09
		;; [21-Sep-2015]: reduce TeX control sequences for
		;; foreign letter glyphs

 		(setq s (bibtex-gsub "{\\AA}"   "AA" s))
 		(setq s (bibtex-gsub "{\\AE}"   "AE" s))
 		(setq s (bibtex-gsub "{\\L}"    "L"  s))
 		(setq s (bibtex-gsub "{\\O}"    "O"  s))
 		(setq s (bibtex-gsub "{\\OE}"   "OE" s))
 		...
 		(setq s (bibtex-gsub "{\\o}"    "o"  s))
 		(setq s (bibtex-gsub "{\\oe}"   "oe" s))
 		(setq s (bibtex-gsub "{\\ss}"   "ss" s))

		;; convert dotless-i to i, dotless-j to j, Polish
		;; slash-l to l, slash-L to L, slash-o to o, slash-O to O
		(setq s (bibtex-gsub "{[\\]\\([ijlLoO] *\\)}" "\\1" s))
		(setq s (bibtex-gsub "[\\]\\([ijlLoO] *\\){}" "\\1" s))
		(setq s (bibtex-gsub "[\\]\\([ijlLoO]\\) +" "\\1" s))
