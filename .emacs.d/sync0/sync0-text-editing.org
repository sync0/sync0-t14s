  ...
  #+BEGIN_SRC emacs-lisp
    ...
    ;;         (centered-window-mode t)))
            ...
            (centered-window-mode t)))
              ...
              (defun sync0-emacs-configuration-centered-window ()
                "Change margins of org-files in the configuration folder."
                (when (derived-mode-p 'text-mode)
        (if  (or ;;(derived-mode-p 'org-agenda-mode)
                 (equal default-directory user-emacs-directory)
                 (equal default-directory sync0-emacs-directory))
            (setq cwm-left-fringe-ratio 30)
            (setq cwm-left-fringe-ratio 100))))

            (add-hook 'find-file-hook 'sync0-emacs-configuration-centered-window)
            (add-hook 'buffer-list-update-hook 'sync0-emacs-configuration-centered-window)

                    :hook 
            ((text-mode . centered-window)
             (prog-mode . sync0-prog-mode-centered-window)))
